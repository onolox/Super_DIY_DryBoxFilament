...
void loop() {
    if (estado == 1) {
        float diffTemp = tempAtual - temperaturaMaterialAtual;
        ...
        if (inicialmente) {  // In the begining maintain power in 100%
            setPotencia(100);

            if (diffTemp >= tempCorte) {
                inicialmente = false;

                if (temperaturaMaterialAtual > 55) {  // PETG
                    pidLiberado = true;
                    setPotencia(65);
                ...
                playButtonTone();
                debugLn("Fim segurar temp");
            }
        }
        else {                   // ----------------------------Start of the Main loop
            if (!pidLiberado) {  // Maintain manually for an hour
                if (tempAtualEhMenor() && cicleLess) {
                    if (diffTemp > 0.4) setPotencia(40);
                    else setPotencia(100 - offsetTemp);
                ...
                else if (tempAtualEhMaior() && cicleMore) {
                    if (!enteredCicleMore) {
                        enteredCicleMore = true;
                        minTemp = ultimaTemp;
                        Serial1.printf("- 21 - %f -- %f", maxTemp, minTemp);
                    }

                    if (diffTemp >= tempCorte) {
                        setPotencia(offsetTemp);  // zero
                        offsetTemp += 10;
                        tempCorte = tempCorte - 0.03;
                        debug(" -2- ");
                        cicleLess = true;
                        cicleMore = false;
                        ...
                        if (maxTemp - minTemp < 0.4) {
                            ...
                            Serial1.printf("- 2 - %f -- %f", maxTemp, minTemp);
                        }
                    }
                }
...
void alterarTipoMaterial() {
    ...
    if (tipoMaterialAtual < 0) tipoMaterialAtual = tamanhoArray;
    else if (tipoMaterialAtual > tamanhoArray) tipoMaterialAtual = 0;
    ...
    if (temperaturaMaterialAtual > 55) tempCorte = 0.2;
    else tempCorte = -0.2;
...
void verificarDHT22() {
    if ((millis() - measurement_timestamp) > 3000) {
        ...
        if (mySensor.read() == DHTLIB_OK) {
            ...
            tempAtual = round(mySensor.getTemperature() * 10) / 10;

            temp1 = temp2;  // Making an average
            temp2 = temp3;
            temp3 = tempAtual;
            tempAtual = round(floatToIntX10((temp1 + temp2 + temp3) / 3)) / 10;
        }
    }
}
...
void setPotencia(int potencia) {
    ...
    dim = map(arrayCompensacao[potenciaAtual], 0, 100, 128, 0);
}  // O dimmer vai de 128 a 0
...
void verificarSeguranca() {
    ...
    if (floatToIntX10(tempAtual) != floatToIntX10(ultimaTemp)) {  // Verifica se a temperatura nao esta mais sendo alterada
...
bool tempAtualEhMenor() { return floatToIntX10(tempAtual) < floatToIntX10(ultimaTemp) ? true : false; }
bool tempAtualEhMaior() { return floatToIntX10(tempAtual) > floatToIntX10(ultimaTemp) ? true : false; }
